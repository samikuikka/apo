        ...
        ? judgeTracer.step(
            {
              ...
              summarize: (r: unknown) => {
                ...
                const pass = res?.verdict?.pass ?? null;
                const reasoning = res?.verdict?.reasoning?.slice(0, 2000)
                  ?? `session ended without a verdict (${res?.outcome ?? "unknown"})`;
                // Lead with the verdict so pass/fail is visible in any text
                // renderer; the reasoning follows.
                return {
                  text: `${pass === null ? "NO VERDICT" : pass ? "PASS" : "FAIL"} — ${reasoning}`,
                };
              },
            },
            run,
